...
class WorkoutService {
    ...
    async getWorkouts(userId) {
        const response = await axiosInstance.get("/" + userId + "/workouts", { headers: authHeader() });
        return response.data;
    }

    async getWorkout(userId, workoutId) {
        const response = await axiosInstance.get("/" + userId + "/workouts/" + workoutId, { headers: authHeader() });
        return response.data;
    }

    async getCurrentWorkout(userId) {
        const response = await axiosInstance.get("/" + userId + "/workouts/info/current", { headers: authHeader() });
        return response.data;
    }
